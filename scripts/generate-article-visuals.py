...
def figure_html(src: str, alt: str, caption: str, hero: bool) -> str:
    ...
    fig = f'{img}\n  <figcaption class="agv-cap"><span class="agv-chip">Editorial illustration — AI-generated (Gemini)</span> <span>{esc(caption)}</span> <span>Editorial only &mdash; does not depict or substantiate reviewed-tool performance.</span></figcaption>'
    if hero:
